...
export async function unsubscribeFromPushNotifications(): Promise<boolean> {
  try {
    ...
    // Remove from server — surface errors so callers can handle them
    ...
    if (!response.ok) {
      // Browser subscription is already revoked; log but still return true
      // so the UI updates. The server record will expire naturally.
