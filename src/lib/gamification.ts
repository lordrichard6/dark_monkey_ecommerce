...
export async function processXpForPurchase(
  ...
  const { data: profile } = await supabase
    .from('user_profiles')
    .select('total_xp, total_spent_cents, total_orders')
    .eq('id', userId)
    .single()
...
export async function processXpForReferral(
  supabase: SupabaseClient,
  referrerId: string
): Promise<AwardXpResult> {
      ...
      if (achievement.points_reward > 0) {
        ...
        if (currentProfile) {
          await supabase
            .from('user_profiles')
            .update({
              total_xp: (currentProfile.total_xp ?? 0) + achievement.points_reward,
            })
            .eq('id', userId)
        }
      }
