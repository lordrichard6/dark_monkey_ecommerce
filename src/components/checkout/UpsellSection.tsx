...
export function UpsellSection({ upsellItems, onAdd, loadingId }: UpsellSectionProps) {
            ...
            >
              ...
              <div className="flex-1">
                ...
                <p className="mt-1 text-xs text-zinc-400">
                  {t('upsellPitch')}{' '}
                  {item.discountPercentage && `${item.discountPercentage}% ${t('off')}`}
                </p>
              </div>

              <div className="flex items-center justify-between gap-4 sm:flex-col sm:items-end">
                <div className="flex items-baseline gap-2">
                  <span className="text-lg font-bold text-zinc-200">{format(item.priceCents)}</span>
                </div>
                <button
                  type="button"
                  onClick={() => !isDisabled && onAdd(item.id)}
                  disabled={isDisabled}
                  className="flex items-center gap-2 rounded-lg bg-white/10 px-4 py-2 text-xs font-bold text-white transition-colors hover:bg-white hover:text-black disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isLoading ? (
                  ...
                  {isLoading ? t('adding') : t('addToOrder')}
                </button>
              </div>
            </div>
          )
