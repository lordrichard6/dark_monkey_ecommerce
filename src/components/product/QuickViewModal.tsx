    ...
    if (isOpen && slug) {
      ...
      getProductQuickView(slug).then((res) => {
        if (res.data) {
          ...
          setCustomizationRule(
            res.customizationRule
              ? (res.customizationRule as { rule_def: CustomizationRuleDef }).rule_def
              : null
          )
        }
        setLoading(false)
  ...
  const modalContent = (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div
        ...
        className="relative w-full max-w-5xl max-h-[90vh] overflow-hidden rounded-2xl border border-white/10 bg-zinc-950/95 shadow-2xl animate-in fade-in zoom-in-95 duration-200"
      >
        <button
          onClick={onClose}
          ...
          {loading ? (
            ...
            <div className="grid gap-8 md:grid-cols-2">
              ...
              <div className="flex flex-col">
                <h2 id="quickview-title" className="text-2xl font-bold text-white md:text-3xl">
                  {product.name}
                </h2>
            ...
            <div className="flex min-h-[400px] flex-col items-center justify-center gap-4">
              ...
              <button onClick={onClose} className="text-sm text-amber-500 hover:underline">
                Go back
              </button>
            </div>
          )}
