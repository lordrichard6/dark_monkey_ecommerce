...
export function ProductImageGallery({
  ...
  // Crossfade key — increments on image change to trigger fade-in animation
  ...
  const selectImage = useCallback((img: ImageItem) => {
  ...
  const thumbnailButtons = unique.map((img, i) => (
    <button
      key={`${img.url}-${i}`}
      type="button"
      onClick={() => selectImage(img)}
      style={THUMBNAIL_BG}
      className={`relative aspect-square w-full shrink-0 overflow-hidden rounded-lg border-2 transition-all duration-200 ease-[cubic-bezier(0.32,0.72,0,1)] ${
        safeIndex === i
    ...
    >
      ...
      {img.video_url && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/40">
          <Play className="h-3.5 w-3.5 text-white" strokeWidth={1.5} />
        </div>
      )}
    </button>
  ))
  ...
  return (
    <div className={`flex flex-col gap-3 ${className}`}>
      {/* Main layout: vertical thumbs LEFT + main image RIGHT on desktop */}
      <div className={showThumbnails && unique.length > 1 ? 'flex gap-3' : ''}>
        {/* Vertical thumbnail strip — desktop only */}
        {showThumbnails && unique.length > 1 && (
          <div className="flex flex-col gap-2 w-[72px] shrink-0 max-h-[520px] overflow-y-auto scrollbar-hide">
            {thumbnailButtons}
          </div>
        )}
        ...
        <div
          style={MOCKUP_BG}
          className="relative flex-1 group overflow-hidden rounded-2xl border border-white/10 backdrop-blur-sm"
          onTouchStart={(e) => {
          ...
          onTouchEnd={(e) => {
            ...
            if (Math.abs(diff) > 40) {
              if (diff > 0) {
                selectImage(unique[(safeIndex + 1) % unique.length])
              } else {
                selectImage(unique[(safeIndex - 1 + unique.length) % unique.length])
              }
            }
            setTouchStartX(null)
          }}
        >
          <div className="relative aspect-square w-full sm:aspect-[4/5]">
            {hasVideo && isPlayingCurrent && currentItem.video_url ? (
              ...
              <>
                {/* Crossfade: key change triggers fade-in animation */}
                <div key={fadeKey} className="absolute inset-0 animate-in fade-in duration-300">
                  <div className="cursor-zoom-in h-full" onClick={() => openLightbox(safeIndex)}>
                    <ProductImageWithFallback
                ...
                {hasVideo && currentItem.video_url && (
                  <button
                    ...
                    className="absolute inset-0 flex items-center justify-center bg-black/30 opacity-0 group-hover:opacity-100 transition-opacity duration-200 ease-[cubic-bezier(0.32,0.72,0,1)]"
                    aria-label="Play video"
                  >
                    <div className="flex h-14 w-14 items-center justify-center rounded-full bg-white/90 text-zinc-950 shadow-xl">
                      <Play className="h-6 w-6 translate-x-0.5" strokeWidth={1.5} />
                    </div>
                  </button>
                )}

                {!hasVideo && (
                  <div className="absolute right-3 top-3 rounded-full bg-black/50 p-2 text-white/70 opacity-0 transition-opacity duration-200 ease-[cubic-bezier(0.32,0.72,0,1)] group-hover:opacity-100 backdrop-blur-md border border-white/10 pointer-events-none">
      ...
      {/* Dot indicators — mobile only (when showThumbnails=false) */}
      {!showThumbnails && unique.length > 1 && (
        <div className="flex justify-center gap-1.5 mt-2">
          {unique.map((_, i) => (
            <button
              key={i}
              onClick={() => selectImage(unique[i])}
              className={`rounded-full transition-all duration-200 ease-[cubic-bezier(0.32,0.72,0,1)] ${
                i === safeIndex
      ...
      {isLightboxOpen && (
        ...
        >
          <button
            ...
            className="absolute right-6 top-6 z-10 rounded-full bg-white/10 p-3 text-white transition-colors duration-200 ease-[cubic-bezier(0.32,0.72,0,1)] hover:bg-white/20 border border-white/10"
          >
          ...
          {unique.length > 1 && (
            <>
              <button
                ...
                className="absolute left-2 sm:left-6 z-10 rounded-full bg-white/10 p-2 sm:p-4 text-white transition-colors duration-200 ease-[cubic-bezier(0.32,0.72,0,1)] hover:bg-white/20 border border-white/10"
              >
              ...
              <button
                ...
                className="absolute right-2 sm:right-6 z-10 rounded-full bg-white/10 p-2 sm:p-4 text-white transition-colors duration-200 ease-[cubic-bezier(0.32,0.72,0,1)] hover:bg-white/20 border border-white/10"
              >
            ...
            <div className="absolute bottom-8 left-1/2 flex max-w-full -translate-x-1/2 gap-3 overflow-x-auto px-6 pb-2 scrollbar-hide">
              {unique.map((img, i) => (
                <button
                  ...
                  className={`relative h-20 w-16 shrink-0 overflow-hidden rounded-xl border-2 transition-all duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] ${
                    i === lightboxIndex
