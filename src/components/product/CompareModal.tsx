...
export function CompareModal({ isOpen, onClose, products }: CompareModalProps) {
  ...
  const features = [
    { label: 'Price', key: 'price_cents', format: (v: number) => format(v) },
    {
      label: 'Category',
      key: 'categories',
      get: (p: Product) => (p.categories as { name?: string } | null)?.name || '-',
    },
    {
      label: 'Rating',
      key: 'rating',
      render: (p: Product) => (
        <div className="flex items-center gap-1">
          ...
          <span className="text-sm">{(p as { rating?: number }).rating || 'New'}</span>
        </div>
      ),
    },
                  ...
                  <th key={product.id} className="w-1/4 p-4 align-top">
                    <div className="space-y-4">
                      ...
                      <Link
                        ...
                        className="block text-sm font-bold text-white hover:text-amber-500"
                      >
                        {product.name}
                      </Link>
                    </div>
            ...
            <tbody className="divide-y divide-white/5">
              {features.map((feature) => (
                <tr key={feature.label} className="group hover:bg-white/5 transition-colors">
                  <td className="p-4 text-xs font-bold uppercase tracking-wider text-zinc-500">
                    {feature.label}
                  </td>
                  {products.map((product) => (
                    <td
                      key={product.id}
                      className={`p-4 text-sm text-zinc-300 ${feature.className || ''}`}
                    >
                      {feature.render
                        ...
                        : feature.get
                          ...
                          : feature.format
                            ? feature.format(
                                (product as unknown as Record<string, number>)[feature.key]
                              )
                            : (product as unknown as Record<string, string>)[feature.key] || '-'}
                    </td>
                  ))}
                  {Array.from({ length: 3 - products.length }).map((_, i) => (
