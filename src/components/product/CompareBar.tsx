            ...
            <div className="flex items-center gap-4 overflow-x-auto pb-1 md:pb-0 no-scrollbar">
              ...
              <div className="flex gap-3">
                {products.map((product) => (
                  <div
                    key={product.id}
                    className="group relative h-14 w-14 shrink-0 overflow-hidden rounded-lg bg-zinc-800 border border-white/5"
                  >
                    <Image
                ...
                {Array.from({ length: 3 - products.length }).map((_, i) => (
                  <div
                    key={i}
                    className="flex h-14 w-14 items-center justify-center rounded-lg border-2 border-dashed border-white/5 bg-white/5 text-zinc-600"
                  >
