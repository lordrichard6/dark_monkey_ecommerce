...
import { testPrintfulConnection } from '@/actions/test-printful'
import { CheckCircle2, RotateCw, XCircle } from 'lucide-react'

export function PrintfulTestButton() {
  ...
  async function handleTest() {
    ...
    setResult(null)
    try {
      const res = await testPrintfulConnection()
      setResult(res)
    } catch (err) {
  ...
  return (
    <div className="rounded-lg border border-zinc-800 bg-zinc-900/80 p-6">
      <div className="flex items-center justify-between">
        <p className="text-sm text-zinc-400">Printful Connection</p>
      ...
      {!result ? (
        ...
        <div>
          ...
          <p className="text-xs text-zinc-500 font-mono mt-1">Token Prefix: {result.tokenPrefix}</p>
        </div>
      ) : (
        <div>
