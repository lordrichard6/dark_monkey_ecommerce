...
export async function createTag(name: string): Promise<{ ok: true; tag: { id: string; name: string } } | { ok: false; error: string }> {
    try {
        ...
        // Check if exists
        const { data: existing } = await supabase.from('tags').select('id, name').eq('slug', slug).single()
...
export async function getGalleryItems(
    ...
    let userVotesMap = new Set<string>()
...
export async function voteForItem(itemId: string, fingerprint?: string): Promise<{ ok: boolean; error?: string }> {
...
export async function uploadGalleryItem(
    ...
    // Check/Create bucket if needed (reusing product-images or creating gallery)
    // Let's use 'product-images' but in a 'gallery' folder for simplicity as that bucket is public and established
    // actually, let's just put it in product-images under gallery/ path
...
export async function deleteGalleryItem(id: string): Promise<{ ok: true } | { ok: false; error: string }> {
    const user = await getAdminUser()
    if (!user) return { ok: false, error: 'Unauthorized' }
