  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-zinc-50">{t('discounts.title')}</h1>
        <Link
          ...
          <div key={d.id} className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4">
            ...
            {d.min_order_cents > 0 && (
              <div className="mt-3 border-t border-zinc-800/50 pt-3 flex justify-between items-center">
                <span className="text-xs text-zinc-500">{t('discounts.minOrderShort')}</span>
                <span className="text-xs font-medium text-zinc-400">
                  {formatPrice(d.min_order_cents)}
                </span>
              </div>
            )}
          </div>
            ...
            <tbody className="divide-y divide-zinc-800/50">
              {(discounts ?? []).map((d) => (
                <tr key={d.id} className="group hover:bg-zinc-800/20 transition-colors">
                  <td className="whitespace-nowrap px-6 py-4 font-mono font-bold text-zinc-50">
                    {d.code}
                  </td>
                  <td className="whitespace-nowrap px-6 py-4 text-sm text-zinc-400 capitalize">
                    {d.type}
                  </td>
                  <td className="whitespace-nowrap px-6 py-4 text-sm font-bold text-amber-500">
                    {d.type === 'percentage'
                      ? `${d.value_cents / 100}%`
                      : formatPrice(d.value_cents)}
                  </td>
