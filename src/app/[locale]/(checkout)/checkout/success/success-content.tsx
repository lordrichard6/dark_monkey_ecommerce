...
export function SuccessContent({ sessionId, initialOrder }: Props) {
  ...
  useEffect(() => {
    ...
    const maxAttempts = 30 // ~90 seconds (Vercel cold start + webhook processing)
  ...
  if (isPolling) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-24 text-center">
        ...
        <h1 className="mb-4 text-2xl font-bold text-zinc-50">{t('orderFinalizing')}</h1>
        <p className="text-zinc-400">{t('orderWait')}</p>
      </div>
    )
  }
