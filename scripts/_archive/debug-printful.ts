  ...
  try {
    ...
    if (res.status === 200) {
      ...
      data.result.forEach((store: any) => {
        console.log(
          `   - ID: ${store.id} | Name: "${store.name}" | Type: ${store.type} | URL: ${store.website}`
        )
      })

      if (data.result.length > 1) {
        console.warn(
          '⚠️  MULTIPLE STORES FOUND. You SHOULD set PRINTFUL_STORE_ID in .env.local to target the correct one.'
        )
      }
    } else {
  ...
  try {
    ...
    if (res.status === 200 && data.result?.length > 0) {
      ...
      if (vData.result?.sync_variants?.length > 0) {
        ...
        console.log(
          `   Selected Test Variant ID: ${testVariantId} (${vData.result.sync_variants[0].name})`
        )
      }
    }
  } catch (e) {
