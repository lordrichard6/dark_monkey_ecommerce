...
test.describe('Admin Panel Access', () => {
  // Admin credentials must be set as env vars — no hardcoded fallbacks
  const adminEmail = process.env.ADMIN_EMAIL ?? ''
  const adminPassword = process.env.ADMIN_PASSWORD ?? ''
  if (!adminEmail || !adminPassword) {
    test.skip(true, 'ADMIN_EMAIL and ADMIN_PASSWORD env vars are required for this test')
  }
  ...
  test('admin login and dashboard access', async ({ page }) => {
    ...
    const isAdmin = await page
      .locator('text=/admin|dashboard|products|orders/i')
      .isVisible()
      .catch(() => false)
  ...
  test('admin product management', async ({ page }) => {
    ...
    const hasProducts = await page
      .locator('text=/product|name|price/i')
      .isVisible({ timeout: 5000 })
      .catch(() => false)
    const hasAddButton = await page
      .locator('button:has-text("Add"), a:has-text("New")')
      .isVisible()
      .catch(() => false)
  ...
  test('admin order management', async ({ page }) => {
    ...
    const hasOrders = await page
      .locator('text=/order|status|total/i')
      .isVisible({ timeout: 5000 })
      .catch(() => false)
  ...
  test('admin stock management', async ({ page }) => {
    ...
    const hasInventory = await page
      .locator('text=/stock|inventory|quantity/i')
      .isVisible({ timeout: 5000 })
      .catch(() => false)
    const hasProducts = await page
      .locator('text=/product/i')
      .isVisible()
      .catch(() => false)
  ...
  test('admin discount codes management', async ({ page }) => {
    ...
    const hasDiscounts = await page
      .locator('text=/discount|code|percentage/i')
      .isVisible({ timeout: 5000 })
      .catch(() => false)
  ...
  test('admin review moderation', async ({ page }) => {
    ...
    const hasReviews = await page
      .locator('text=/review|rating|approve/i')
      .isVisible({ timeout: 5000 })
      .catch(() => false)
    const hasEmptyState = await page
      .locator('text=/no reviews/i')
      .isVisible()
      .catch(() => false)
