...
test.describe('Account Management', () => {
  ...
  test('signup flow - new user registration', async ({ page }) => {
    ...
    const hasConfirmMessage = await page
      .locator('text=/confirm|check your email/i')
      .isVisible()
      .catch(() => false)
  ...
  test('profile page - view user information', async ({ page }) => {
    ...
    const hasProfileInfo = await page
      .locator('text=/profile|account|email/i')
      .isVisible()
      .catch(() => false)
  ...
  test('gamification - view XP and tier', async ({ page }) => {
    ...
    const hasTierInfo = await page
      .locator('text=/bronze|silver|gold|vip|tier|xp/i')
      .isVisible()
      .catch(() => false)
    const hasLoginPrompt = await page
      .locator('text=/log in/i')
      .isVisible()
      .catch(() => false)
  ...
  test('wishlist - add and remove items', async ({ page }) => {
    ...
    const wishlistButton = page
      .locator('button[aria-label*="wishlist"], button[title*="wishlist"]')
      .first()
  ...
  test('order history - view past orders', async ({ page }) => {
    ...
    const hasEmptyState = await page
      .locator("text=/no orders|haven't made/i")
      .isVisible()
      .catch(() => false)
  ...
  test('login flow - invalid credentials show error message', async ({ page }) => {
    await page.goto('/login')
    await expect(page.getByRole('heading', { name: /log in|sign in/i })).toBeVisible()

    // Fill in clearly wrong credentials
    await page.locator('input[type="email"]').fill('nonexistent-user@invalid-domain-xyz.com')
    await page.locator('input[type="password"]').fill('WrongPassword999!')

    await page.getByRole('button', { name: /log in|sign in/i }).click()

    // Error message should appear (rendered in #auth-message with text-red-400)
    await page.waitForTimeout(3000)
    const errorMessage = page.locator('#auth-message')
    await expect(errorMessage).toBeVisible({ timeout: 5000 })
    // Error element should contain some indication of failure
    const errorText = await errorMessage.textContent()
    expect(errorText).toBeTruthy()
  })

  test('login flow - empty form submission shows validation', async ({ page }) => {
    await page.goto('/login')
    await expect(page.getByRole('heading', { name: /log in|sign in/i })).toBeVisible()
    ...
    // Should show validation: either HTML5 required validation prevents submit,
    // or the app shows an error message — either way, we should still be on /login
    const currentUrl = page.url()
  ...
  test('forgot password flow - sends reset email', async ({ page }) => {
    ...
    // Fill in a valid-looking email address
    const emailInput = page.locator('input[type="email"]')
    ...
    // After submit, should show a success confirmation ("check your email" state)
    // The form switches to a success view with checkEmailReset translation key
    await page.waitForTimeout(3000)
    const hasSuccessState = await page
      .locator('text=/check your email|email sent|reset link/i')
      .isVisible()
      .catch(() => false)
    ...
    // Should show success (email sent) not an error
    // Note: even with a non-existent email, Supabase returns success to prevent user enumeration
    expect(hasSuccessState || !hasErrorState).toBeTruthy()
  })

  // NOTE: Email confirmation flow cannot be tested E2E without access to a real mailbox.
  // Supabase requires clicking a confirmation link sent to the registered email address.
  // To enable full signup E2E testing, either:
  //   a) Disable email confirmation in Supabase Dashboard → Auth → Email → "Confirm email" toggle (development only)
  //   b) Use a test email service (Mailhog, Mailtrap) and parse the confirmation link
  // test.fixme('signup flow - email confirmation', () => {})
})
